...
| **Status:** Systems ALIVE, Phase 1 ✅ (eco unfrozen, Q fed, Valhalla live), $0 earned, **30 RustChain PRs MERGED** (May 27-28), 30/300 gaps closed
...
| T10 | **No remote state persistence** | All state local — one disk failure = full loss | P1 |
...
| P5 | **No options flow data** | Put/call ratios, max pain, IV skew → SP500 signal | P0 |
| P6 | **No order book features** | Level 2 data for BTC — detect spoofing, accumulation | P0 |
| P7 | **No news sentiment features** | GDELT headlines → structured features → WR boost | P0 |
| P8 | **No macro event features** | FOMC, CPI, NFP → event-window features | P0 |
| P9 | **No cross-asset features** | BTC-SP500 correlation regime, VIX-BTC divergence | P0 |
| P10 | **No temporal sequence model** | Current MLP doesn't model time — need LSTM/Transformer | P0 |
| P11 | **No multi-timeframe features** | All features compute at one resolution — no φ scaling | P1 |
| P12 | **GAAD golden ratio not implemented** | φ-based intervals from paper — never coded | P1 |
| P13 | **DFT features not implemented** | Frequency-domain features from DFT-WuBu paper | P1 |
| P14 | **DCT features not implemented** | Compressed market state from DCT-WuBu paper | P1 |
| P15 | **Tailslayer hedging not implemented** | Beam-search speculative decoding for ensemble execution | P1 |
| P16 | **No feature importance tracking** | Can't see which of 13 features contribute to WR | P1 |
...
| P19 | **No hyperparameter search** | Learning rate, hidden dim, architecture — all guessed | P1 |
| P20 | **No regularization** | No dropout, weight decay, early stopping | P1 |
...
| C2 | **P2P matching is zero-sum** | Winners take from losers — total portfolio never grows | P1 |
| C3 | **Market-mode not live** | MARKET_MODE only tested in paper — never live with real feeds | P1 |
| C4 | **No capital redistribution pool** | Darwin culls agents, redistributes capital — pool fractions unverified | P1 |
| C5 | **No edge threshold in live mode** | Agents trade on any signal — no min conviction filter | P1 |
| C6 | **No fee model in C engine live** | P2P with FEE_RATE=0.001 but never deducted from capital | P1 |
| C7 | **Capital tracking uses float** | Float rounding errors accumulate over 26K+ cycles | P1 |
| C8 | **No capital baseline drift tracking** | Can't measure "did the room make money" vs noise | P1 |
| C9 | **No historical PnL per agent** | Per-agent PnL not persisted — can't identify alpha genes | P1 |
| C10 | **No gene-pool diversity metrics** | Can't measure if Darwin is converging to mono-culture | P1 |
| C11 | **No convinction accuracy tracking** | Don't know if agents are betting with conviction or gambling | P1 |
| C12 | **No feature weight convergence** | Learned weights in v2 have no convergence metric | P1 |
...
| C19 | **No win rate per room** | Only aggregate across all rooms and agents | P2 |
...
| E5 | **No teacher process monitoring** | No cron/daemon ensuring teachers stay alive | P1 |
| E6 | **No room health monitoring** | No automatic restart if room_engine crashes | P1 |
| E7 | **No heartbeat for 3/4 rooms** | macro, momentum, polymarket have .heartbeat files but stale | P1 |
| E8 | **room_feed_bridge error handling** | No retry on failure — single crash kills feed | P1 |
| E9 | **No Q-controller reward feedback** | Q-values near zero — no trade result fed back | P1 |
| E10 | **Ecosystem not feeding trade results** | Trades happen but rewards don't propagate to Q-table | P1 |
| E11 | **No ecosystem state checkpoint** | eco/minute_log.jsonl stops at May 26 | P1 |
| E12 | **No ecosystem restart automation** | Must manually restore from minute_log.jsonl after crash | P1 |
| E13 | **No SQLite for ecosystem** | minute_log.jsonl has no query capability | P1 |
| E14 | **No trade journal** | Individual trades not logged in searchable format | P1 |
| E15 | **No order book archive** | Level 1 data only — no bid/ask history | P1 |
| E16 | **No data quality checks** | No validation on incoming price data | P1 |
