/**
 * mlb_advanced_collector.c — MLB Advanced Stats (pitching/batting splits)
 * Compile: gcc -O2 -Wall -o mlb_advanced_collector mlb_advanced_collector.c -lcurl -ljansson -lsqlite3 -lm
 * Usage: ./mlb_advanced_collector
 */
...
#define OUT_FILE OUT_DIR "/mlb_advanced.json"
...
int main(void) {
    ...
    sqlite3_exec(db,"CREATE TABLE IF NOT EXISTS mlb_advanced (team TEXT,stat_name TEXT,stat_value TEXT,fetched_at INTEGER,PRIMARY KEY(team,stat_name))",NULL,NULL,NULL);
    json_t *root = json_array();
    char *resp = get("https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/teams");
    if (!resp) return 1;
    ...
    json_decref(j); printf("[MLB] %d teams\n", n);
    for(int t=0;t<n;t++) {
        char url[256]; snprintf(url,sizeof(url),"https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/teams/%d/statistics",ids[t]);
        ...
        json_array_foreach(cats,ci,cat) {
            ...
            json_array_foreach(cs,si,st) {
                ...
                sqlite3_stmt *st2;
                if(sqlite3_prepare_v2(db,"INSERT OR REPLACE INTO mlb_advanced VALUES(?,?,?,?)",-1,&st2,NULL)==SQLITE_OK){
