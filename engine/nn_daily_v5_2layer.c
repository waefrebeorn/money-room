...
#define N_INPUTS    13  // Keep original features — model architecture is the ceiling
#define N_HIDDEN_1  32  // Wider first layer
#define N_HIDDEN_2  16  // Second hidden layer
#define N_OUTPUTS   1
...
typedef struct {
    float w1[N_INPUTS * N_HIDDEN_1];
    float b1[N_HIDDEN_1];
    float w2[N_HIDDEN_1 * N_HIDDEN_2];
    float b2[N_HIDDEN_2];
    float w3[N_HIDDEN_2 * N_OUTPUTS];
    float b3[N_OUTPUTS];
} NNWeights;
...
static float nn_forward(const NNWeights *w, const float *feat, float *logit_out) {
    ...
    for (int i = 0; i < N_HIDDEN_1; i++) {
        float s = w->b1[i];
        for (int j = 0; j < N_INPUTS; j++) s += feat[j] * w->w1[j * N_HIDDEN_1 + i];
        h1[i] = relu(s);
    }
    float h2[N_HIDDEN_2];
    for (int i = 0; i < N_HIDDEN_2; i++) {
        float s = w->b2[i];
        for (int j = 0; j < N_HIDDEN_1; j++) s += h1[j] * w->w2[j * N_HIDDEN_2 + i];
        h2[i] = relu(s);
    }
    float logit = w->b3[0];
    for (int j = 0; j < N_HIDDEN_2; j++) logit += h2[j] * w->w3[j];
...
static void nn_reinforce(NNWeights *w, int won, float stake, float logit, float lr) {
    ...
    float grad = lr * advantage * stake;
    // Propagate gradient through h2
    float dh2[N_HIDDEN_2];
    for (int i = 0; i < N_HIDDEN_2; i++) {
        dh2[i] = grad * w->w3[i];
        w->w3[i] += grad * 0.1f;  // smaller LR on output layer
    }
    w->b3[0] += grad;
    // Backprop through h1→h2
    for (int i = 0; i < N_HIDDEN_1; i++) {
        float dh1 = 0;
        for (int j = 0; j < N_HIDDEN_2; j++) dh1 += dh2[j] * w->w2[i * N_HIDDEN_2 + j];
        if (dh1 > 0) { // ReLU derivative
            for (int j = 0; j < N_HIDDEN_2; j++)
                w->w2[i * N_HIDDEN_2 + j] += grad * 0.1f;
        }
    }
    for (int i = 0; i < N_HIDDEN_2; i++) w->b2[i] += grad * 0.05f;
    // Backprop through input→h1
    for (int i = 0; i < N_INPUTS; i++) {
        float dh1 = 0;
        for (int j = 0; j < N_HIDDEN_1; j++) dh1 += 0;  // simplified
    }
    // Simple update: move weights toward better outcomes
    for (int i = 0; i < N_HIDDEN_1; i++) w->b1[i] += grad * 0.01f;
}
...
// ════════════════════════════════════════════════════════
//  FEATURE ENGINE (daily SP500 + VIX + BTC + φ-features)
// ════════════════════════════════════════════════════════
// Returns 16-dim feature vector from history up to span-1.
// Features strictly BEFORE the target day.
static void compute_daily_features(const float *px, const float *vx,
                                           const float *vd, const float *bx,
                                           int len,
    ...
    // ── P1: Non-OHLCV features to break 55% ceiling ──
    // F14: BTC 30-day return (risk-on/risk-off regime signal)
    if (len >= 31 && bx[len-1] > 0) {
        float btc_prev = bx[len-31];
        feat[13] = btc_prev > 0 ? (bx[len-1] - btc_prev) / btc_prev * 100.0f : 0;
        // Clamp to ±100%
        if (feat[13] > 100.0f) feat[13] = 100.0f;
        if (feat[13] < -100.0f) feat[13] = -100.0f;
    }

    // F15: BTC-SP500 correlation over last 10 days
    if (len >= 11) {
        float sp_mean = 0, bt_mean = 0;
        for (int i = len-10; i < len; i++) {
            sp_mean += px[i]; bt_mean += bx[i];
        }
        sp_mean /= 10; bt_mean /= 10;
        float cov = 0, sp_var = 0, bt_var = 0;
        for (int i = len-10; i < len; i++) {
            float sd = px[i] - sp_mean, bd = bx[i] - bt_mean;
            cov += sd * bd; sp_var += sd * sd; bt_var += bd * bd;
        }
        float denom = sqrtf(sp_var * bt_var);
        feat[14] = denom > 0.0001f ? cov / denom : 0.0f;
    }

    // F16: GAAD φ-interval return (multi-scale at φ intervals)
    // Uses φ¹=1.618, φ²=2.618, φ⁵=11.09 day intervals
    for (int phi_exp = 0; phi_exp < 3; phi_exp++) {
        int interval = (int)(powf(PHI, (float)(phi_exp + 1)) + 0.5f);
        if (interval < 2) interval = 2;
        if (len >= interval + 2) {
            float p_prev = px[len - 1 - interval];
            float ret = p_prev > 0 ? (px[len-1] - p_prev) / p_prev * 100.0f : 0;
            // F16 is an aggregate of φ-interval returns
            feat[15] += ret / (float)(phi_exp + 1);
        }
    }
    // Normalize
    feat[15] = fmaxf(-10.0f, fminf(10.0f, feat[15]));
}
...
static int load_daily(const char *sp_path, const char *vix_path,
    ...
    int n_btc = 0;
    f = fopen("/tmp/btc_daily.csv", "r");
    if (f) {
        fgets(line, sizeof(line), f); // header
        while (fgets(line, sizeof(line), f) && n_btc < MAX_DAYS) {
            char dt[32]; float val;
            if (sscanf(line, " %31[^,],%f", dt, &val) >= 2) {
                if (strlen(dt) > 0 && val > 0) {
                    strncpy(btc_dates[n_btc], dt, 15); btc_dates[n_btc][15]=0;
                    btc_buf[n_btc] = val;
                    n_btc++;
                }
            }
    ...
    for (int si = 0; si < n_sp && count < max; si++) {
        for (int vi = 0; vi < n_vix; vi++) {
            if (strcmp(dates[si], vix_dates[vi]) == 0) {
                ...
                // Find matching BTC
                float btc = 0;
                for (int bi = 0; bi < n_btc; bi++) {
                    if (strcmp(dates[si], btc_dates[bi]) == 0) {
                        btc = btc_buf[bi];
                        break;
                    }
                }
...
int main(int argc, char **argv) {
    ...
    printf("=== NN DAILY — SP500+VIX Regime Paper Proof ===\n");
    ...
    float px[MAX_DAYS], vx[MAX_DAYS], vy[MAX_DAYS], bx[MAX_DAYS];
    for (int i = 0; i < nd; i++) {
        px[i] = data[i].sp500; vx[i] = data[i].vix; vy[i] = data[i].yield; bx[i] = data[i].btc;
    }
    ...
    for (int t = WARMUP; t < nd; t++) {
        ...
        compute_daily_features(px, vx, vy, bx, t, features, 16);
